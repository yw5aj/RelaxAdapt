...
class CleanFiber:
    ...
    def __init__(self, mat_filename, threshold=.25, pad=300., make_plot=False):
    ...
    def get_animal_info(self):
        with open('./recordings/notes/ModelGfpInfo0729.csv', 'r') as f:
            animal_info_splitted = re.findall(self.mat_filename+r'.+',
                                              f.read())[0].split(',')
    ...
    def get_mat_data(self):
        self.mat_data = loadmat('./recordings/'+self.mat_filename)
        self.stim_block = []
        i = 0
        while True:
            try:
                self.stim_block.append(
                    self.mat_data[
                        'COUT_PUT_F'+str(self.animal_info['unitid']) +
                        '0' + str(i + 1)].shape[1])
                i += 1
            except:
                break
    ...
    def sort_traces(self):
        ...
        for i, j_num in enumerate(self.stim_block):
            for j in range(j_num):
                ...
                traces_dict['displ'] = traces_dict['displ'] -\
                    traces_dict['displ'][0]
    ...
    def find_contact_by_force(self, threshold, pad):
        ...
        for stim_id, stim_traces_full in enumerate(self.traces_full):
            ...
            try:
                contact_index = np.nonzero(stim_traces_full['force'] >
                                           threshold)[0][0]
            except:
                continue
    ...
    def cut_traces(self, make_plot=False):
        self.traces = []
        for stim_id, stim_traces_full in enumerate(self.traces_full):
            contact_index = np.nonzero(stim_traces_full['displ'] >=
            ...
            self.traces.append({
                ...
                'static_displ': stim_traces_full['displ'][
                    max_force_index + 2 * self.fs:max_force_index + int(
                        ...
                        contact_index],
                'dynamic_force_rate': np.diff(stim_traces_full['force'][
                    contact_index:max_force_index]).mean() * self.fs,
                'dynamic_displ_rate': np.diff(stim_traces_full['displ'][
                    contact_index:max_force_index]).mean() * self.fs,
                })
            try:
                self.traces[-1]['raw_spike'] = stim_traces_full['raw_spike'][
                    contact_index:contact_index + 6 * self.fs]
            except KeyError:
                pass
            ...
            if make_plot:
                ...
                for i, item in enumerate(['displ', 'force', 'spike_trace']):
                    axs[i].plot(stim_traces_full['time'][
                        contact_index - .5 * self.fs:contact_index +
                        6.5 * self.fs], stim_traces_full[item][
                            contact_index - .5 * self.fs:contact_index + 6.5 *
                            self.fs], '-k', color='.0')
...
#                    axs[i].set_xlim(0, 8)
                axs[0].set_title(
                    ...
                    (self.traces[-1]['static_displ'], self.traces[-1][
                        'static_force'], stim_traces_full['spike_trace'][
                            :contact_index].sum()))
...
#        label_text = '#'+str(fiber_id)
        ...
        axs[0].plot(static_displ_list, static_force_list, fmt, color=color,
                    ms=6, label=label_text)
        axs[1].plot(static_displ_list, static_avg_fr_list, fmt, color=color,
                    ms=6, label=label_text)
        axs[2].plot(static_force_list, static_avg_fr_list, fmt, color=color,
                    ms=6, label=label_text)
#        axs[3].plot(static_displ_list, dynamic_avg_fr_list, fmt, color=color,
#                    ms=6, label='#'+str(fiber_id))
...
#    axs[3].set_ylabel(r'FR$_d$ (Hz)')
    # Do legend for the entire figure
    handles, labels = axs[0].get_legend_handles_labels()
    legend = axs[0].legend(
        handles, labels, bbox_to_anchor=(0, 1.4, 1., 0.3),
